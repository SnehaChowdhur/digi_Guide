...
const starters: Message[] = [{ id: 1, role: "model", content: "Hi Alex. I’m NOVA, your computer science tutor. What concept would you like to work through?" }];
...
export default function AiTutor() {
  ...
  return <section className="tutor-panel"><header className="tutor-header"><div className="tutor-title"><div className="tutor-icon"><BrainCircuit size={20} /></div><div><div className="kicker">NOVA tutor</div><h2>Learn by thinking it through.</h2><p>Ask for an explanation, an example, or a practice challenge.</p></div></div><span className="tutor-status"><i /> Gemini Flash ready</span></header><div className="chat-history" aria-live="polite">{messages.map(message => <div className={`chat-message ${message.role}`} key={message.id}><div className="message-avatar">{message.role === "model" ? <Bot size={15} /> : <User size={15} />}</div><div className="message-bubble">{message.content || <span className="typing"><i /><i /><i /></span>}{message.streaming && message.content && <span className="stream-caret" />}</div></div>)}<div ref={bottom} /></div><div className="quick-prompts"><span>Try asking</span>{prompts.map(prompt => <button key={prompt} onClick={() => quickPrompt(prompt)}><Sparkles size={13} />{prompt}</button>)}</div><form className="chat-composer" onSubmit={send}><textarea ref={textarea} rows={1} value={input} placeholder="Ask NOVA anything about computer science..." onChange={event => { setInput(event.target.value); resize(); }} onKeyDown={keyDown} aria-label="Message NOVA" /><button type="submit" disabled={!input.trim() || loading} aria-label="Send message">{loading ? <LoaderCircle className="spin" size={18} /> : <ArrowUp size={18} />}</button></form><p className="composer-note">NOVA can make mistakes. Use it to reason, then verify important details.</p></section>;
}
